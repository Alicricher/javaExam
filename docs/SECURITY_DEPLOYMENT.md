...
- [x] Домен `medotm.uz` куплен, A-запись настроена — сделано 2026-09-03
- [x] Caddy + Let's Encrypt TLS для `medotm.uz`/`www.medotm.uz` — сделано 2026-09-03, автопродление сертификата встроено в Caddy
- [x] `server.forward-headers-strategy=native` — сделано 2026-09-03, кука сессии подтверждённо получает `Secure`
...
Итог: `https://medotm.uz` — рабочий публичный адрес админки. Прямой `http://IP:8080`
недоступен (порт только на loopback), `http://medotm.uz` автоматически редиректит на
HTTPS (308).
